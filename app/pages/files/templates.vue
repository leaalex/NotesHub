...
async function reload() {
  loading.value = true
  try {
    rows.value = await apiFetch<TemplateRow[]>('/api/file-field-templates')
  }
  catch (e) {
    console.error(e)
...
async function addTemplate() {
  ...
  if (!lb)
    return
  await apiFetch<TemplateRow>('/api/file-field-templates', {
    method: 'POST',
...
async function patchRemote(
  ...
  await apiFetch(`/api/file-field-templates/${row.id}`, {
    method: 'PATCH',
    body: patch,
  })
}
