...
cp .env.example .env
mkdir -p data
npm run db:push                # or rely on automatic migrate on server start
npm run dev
```
...
- Manual push (dev): `npm run db:push`
...
На хосте **не нужен** свой `npm install` для запуска: при старте контейнер выполняет `npm install` в Linux и кладёт зависимости в том **`notes_node_modules`**. Исходники монтируются с Mac, а **нативные модули** (например `oxc-parser`) собираются внутри образа.

После смены зависимостей закоммитьте **`package-lock.json`** и пересоберите / перезапустите compose (том подтянет актуальный lockfile).
...
Один проход: установка зависимостей в Linux-томе, `npm test`, затем `npm run build` (как быстрая проверка CI):
...
| `/cms` | Admin read-only views: folders, notes, tasks (SQLite) |
